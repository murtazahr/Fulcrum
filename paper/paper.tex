...
its viability for production-scale next-word prediction. Wei et~al.~\cite{wei2020nbafl} provided the first comprehensive convergence
analysis of DP-FL via the noising-before-aggregation framework, which
informs our utility cost analysis in Section~\ref{sec:defense}.
...
differential privacy alone. This perspective treats topology as a privacy asset for
honest participants. Our work takes the opposite view: we assume the
adversary has global knowledge of the topology and study how
structural position and organisational membership become information
channels for inferring sensitive data properties. Feng
et~al.~\cite{feng2025topology} occupy a third position, demonstrating
that the topology itself can be inferred from model behaviour alone.
Our threat model differs from both: we assume topology is known and
treat it as a leakage surface rather than an amplification mechanism
...
multiclass settings. In cross-silo deployments, $p_i$ encodes population-level attributes
that organisations are legally and ethically obligated to
...
to serve. Attack performance
is measured by calibration loss
$L_{\text{cal}} = \frac{1}{n}\sum_i (\hat{p}_i - p_i)^2$ and attack
lift relative to the constant-mean baseline
...
$\mathcal{G}$, its distance to the topology root, and its betweenness
centrality, with the organisational label $\omega_i$ encoded as a
one-hot vector and concatenated. Distance to root is defined for
rooted topologies such as hierarchical FL; for decentralised
peer-to-peer topologies without a natural root, this feature is set
to zero. Betweenness centrality is precomputed from the known
topology $\mathcal{G}$ prior to training.
...
the shadow-model framework~\cite{shokri2017membership}. The adversary
simulates federations on a public proxy dataset where the ground-truth
$p_i$ is known, harvests $(\Theta_i, x_i, p_i)$ tuples across many
...
consistent with the threat model of Section~\ref{sec:adv-model}. To
assess robustness to the choice of model class, the regressor is
instantiated as gradient-boosted trees (\textsc{LightGBM}, the
default), a small multilayer perceptron, or ridge regression. All
produce predictions clipped to $[0,1]$. Algorithm~\ref{alg:tadi}
gives the formal specification.

\begin{table*}[t]
\centering
\caption{Attack evaluation metrics computed per target run and
channel.}
\label{tab:metrics}
\begin{tabular}{@{}lll@{}}
\toprule
...
Trivial predictor; reference for computing attack lift \\
Attack lift &
$L_{\text{cal}}(\bar{p}, p) - L_{\text{cal}}(\hat{p}, p)$ &
Positive iff adversary beats baseline; zero under IID-null \\
Top-$k$ recovery &
...
Attack performance across the four ablations is evaluated using the
metrics summarised in Table~\ref{tab:metrics}. Here
$\text{top-}k(\hat{p})$ and $\text{top-}k(p)$ denote the sets of
$k$ clients with the highest predicted and true sensitive-class
concentration respectively, and $\tau$ is a concentration threshold
used to binarise the ranking task for AUROC.
...
The channel decomposition introduced in Section~\ref{sec:attack}
separates the adversary's available information into a parameter
component, a structural component, and an organisational component.
As we show formally in this section, the leakage an adversary can
extract about a client's data separates accordingly into a mechanism
term that flows through parameter updates and can be suppressed by
noise, and a prior-coupling term that flows through structural
position and cannot. The defense targets the controllable component.
It controls one knob per client, the noise scale $\sigma_i$ applied
during DP-SGD, and allocates it asymmetrically as a function of
structural-leverage scores. Standard practice sets $\sigma_i$
uniformly across clients, implicitly assuming a symmetric topology.
We prove a per-client mutual-information bound that holds against the adversary of Section~\ref{sec:adv-model} and derive an allocation
that is balanced min-max optimal: it strictly dominates uniform
DP-SGD whenever leverage scores are non-uniform across clients, and
degenerates exactly to uniform DP-SGD when they are not. The
allocation is named \Fulcrum: per-client privacy bounds are
equilibrated across an asymmetric leverage profile at a fixed utility
...
$D = (D_1,\ldots,D_n)$ with class distributions
$\Delta_i = \delta(D_i)$ and sensitive-class concentrations
$p_i = \Delta_i(\mathcal{C}_s)$. DP-SGD~\cite{abadi2016dpsgd}
produces, per round and per client,
\begin{equation}
\begin{split}
...
sampled by client $i$ at round $t$, $\ell$ is the loss function, and
$\xi_i^{(t)} \sim \mathcal{N}(0, \sigma_i^2 C^2 I)$ is the per-round
Gaussian noise introduced by DP-SGD. The adversary observes
$\Theta = \{\theta_i^{(t)}\}_{i,t}$ and outputs estimate $\hat{p}_i$
via a deterministic function $f$ as defined in
...
$\mathcal{F}_{\mathcal{G},\omega}$ over $(\Delta_1,\ldots,\Delta_n)$
satisfying three conditions: (a) the prior factorises over connected
components of $\mathcal{G}$; (b) within each component, the prior
satisfies a Markov property with respect to $\mathcal{G}$; and
(c) the prior has bounded second moments. These conditions are
standard in graphical-model analyses. Condition (c) ensures that the
supremum quantities introduced below are finite, and all three
conditions are satisfied by the deployment priors used in
...
the rest of the federation's data. A client at a structurally exposed
position, such as the hub of a star topology or the sole member of a
small organisational group, carries more lateral information about
$p_i$ than a client in a symmetric or isolated position. We formalise
this intuition through the concept of structural leverage.
...
\;\leq\;
\underbrace{\frac{T_{\max} C^2}{2\sigma_i^2 |B|^2}}_{\text{controllable}}
\;+\;
...
gives $I(p_i;\Theta) \leq I(p_i;\Theta \mid D_{-i}) + I(p_i;D_{-i})$.
Under (IA2), the first term reduces to $I(D_i;\Theta_i \mid D_{-i})$
and is bounded by $T_{\max} C^2/(2\sigma_i^2 |B|^2)$ via
Mironov's RDP composition~\cite{mironov2017rdp} applied to the
Gaussian mechanism~\cite{abadi2016dpsgd}, converted to mutual
information via the max-KL bound of Cuff and
Yu~\cite{cuff2016mi}. The second term is bounded by $\ell_i^\circ$
by Definition~\ref{def:leverage}. The full proof is given in
...
$C^2/(2\sigma_i^2|\mathcal{D}_i|^2)$, a substantial improvement
when $|\mathcal{D}_i| \gg |B|$. Tighter bounds via Asoodeh
et~al.~\cite{asoodeh2021variants} or amplified RDP are natural
extensions of the proof structure and are discussed in Section~\ref{sec:discussion}.
...
clients subject to a fixed total noise budget. Let
$a := T_{\max} C^2 / (2|B|^2)$. The optimisation problem is
\begin{equation}
...
under standard FL convergence
assumptions~\cite{wei2020nbafl},
...
Slater's condition holds at $\sigma_i^2 = U/(2n)$. Karush-Kuhn-Tucker (KKT) conditions therefore give
$\sigma_i^{*\,2} = a/(K^\star - \ell_i^\circ)$ at the active-budget
optimum; uniqueness of $K^\star$ follows from
strict monotonicity of the budget equation in $K$. The full proof
is given in Appendix~\ref{app:thm2}.
\end{proof}

An immediate consequence of Theorem~\ref{thm:allocation} is that the topology-aware allocation strictly dominates uniform DP-SGD whenever the federation is asymmetric.
...
Symmetric topologies such as rings, complete graphs, and balanced hierarchies produce uniform leverage scores, and the allocation reduces to standard uniform DP-SGD. This is the appropriate behaviour: there is no asymmetry to exploit and no penalty for using \Fulcrum unconditionally. Asymmetric topologies such as lines, hierarchies with uneven groups, and stars, as well as deployments with heterogeneous client dataset sizes, produce non-uniform leverage and admit a strict improvement.
...
asymmetry. The first carries a partial analytical derivation; the
remaining two are heuristics validated empirically in
Section~\ref{sec:experiments}.\smallskip
...
proxy is appropriate whenever the dominant source of asymmetry is
uneven organisational group sizes, as in hierarchical FL with
heterogeneous client clusters~\cite{briggs2020hierarchical}.\smallskip

\noindent\textbf{2. Degree proxy.} Under bounded-degree Markov priors
with a regularity assumption on neighbourhood-conditional entropy,
leverage admits an upper bound proportional to graph degree. We adopt
the proxy $\ell_i^{\mathrm{deg}} \propto \deg_{\mathcal{G}}(i)$ as
a heuristic for deployments where the dominant asymmetry arises from
non-uniform graph degree, such as decentralised peer-to-peer
protocols~\cite{roy2019braintorrent,hegedus2021decentralized,beilharz2021dag}.
A tight analytical bound for this proxy remains an open problem and
is discussed in Section~\ref{sec:discussion}.\smallskip

\noindent\textbf{3. Dataset-size proxy.} In FedAvg with
size-weighted aggregation
$\theta^{(t+1)} = \sum_i \frac{|\mathcal{D}_i|}{\sum_j |\mathcal{D}_j|}
\theta_i^{(t+1)}$,
each client's update is weighted proportionally to its dataset size,
making larger clients structurally more influential. We adopt the
mean-normalised proxy
$\ell_i^{\mathrm{ds}} \propto |\mathcal{D}_i|/\bar{|\mathcal{D}|}$
for deployments where the dominant asymmetry arises from
heterogeneous client dataset sizes, as in the FLamby cross-silo
regime~\cite{flamby2022}. Like the degree proxy, this heuristic is
empirically validated rather than analytically proven.\smallskip

\noindent The three proxies are not mutually exclusive. A deployment can blend
them by passing per-client weight vectors that combine group size,
degree, and dataset size, normalised to unit mean. The choice of proxy
is a deployment design parameter; each experimental setting in
Section~\ref{sec:experiments} uses the proxy that matches its
...
We evaluate both contributions on three federated benchmarks spanning the canonical cross-silo deployment regimes. \TADI is evaluated on its ability to recover sensitive-class concentrations across the four channel ablations, and \Fulcrum is evaluated on its privacy-utility trade-off relative to uniform DP-SGD. The benchmarks are chosen to
cover different sources of structural asymmetry:  heterogeneous site sizes in a real healthcare imaging federation, heterogeneous site sizes in a real cross-silo tabular task, and parametric topology-data coupling in a controlled synthetic setting. Together they provide both
deployment realism and the statistical power needed to validate the theoretical claims of Section~\ref{sec:defense}.
...
train a small convolutional network with GroupNorm replacing BatchNorm
for DP-SGD compatibility. This setting combines dataset-size asymmetry
with hierarchical topology structure, so both the dataset-size proxy
and the organisational position of each site within the hierarchy
contribute to the leverage profile.\medskip
...
topology, all nodes share the same degree and the topology introduces
no structural leverage asymmetry. Dataset-size heterogeneity is
therefore the sole source of asymmetry, with the largest centre
contributing more than six times the training samples of the smallest,
making this a clean test of the dataset-size proxy in isolation from
any topological confound.\medskip

\noindent\textbf{Setting C: Synthetic CIFAR-10 with parametric coupling.}
To provide controlled statistical power for validating the theoretical
claims of Section~\ref{sec:defense}, we partition
CIFAR-10~\cite{krizhevsky2009cifar} across $n = 50$ synthetic clients
using the $\eta$-coupled construction
\[
...
position-determined class distributions ($\eta = 1$). Topology is varied across ring, line, hierarchical, star,
Erd\H{o}s-R\'{e}nyi~\cite{erdos1959random}, and
Barab\'{a}si-Albert~\cite{barabasi1999emergence} configurations to
expose the full leverage-asymmetry spectrum, from uniform-degree
graphs to scale-free networks with highly non-uniform degree
distributions. Setting~C is
the only configuration in which AUROC is a meaningful metric
($n \geq 20$) and provides clean control over $\eta$ as the parameter
driving lateral leakage.
...
\midrule
$n$ & Number of clients \\
$\mathcal{P} = \{P_1, \ldots, P_n\}$ & Set of clients \\
$\mathcal{G} = (\mathcal{P}, E)$ & Communication topology \\
$E$ & Set of edges in the communication topology \\
$\mathcal{D}_i$ & Local dataset of client $i$ \\
$\mathcal{C}$ & Class set \\
$K = |\mathcal{C}|$ & Number of classes \\
$\Delta_i \in \Delta(\mathcal{C})$ & Local class distribution of client $i$ \\
$\omega : \mathcal{P} \to [k_{\mathrm{org}}]$ & Organisational labelling \\
$k_{\mathrm{org}}$ & Number of distinct organisational groups \\
$T_{\max}$ & Number of training rounds \\
$\sigma_i$ & Per-client DP-SGD noise scale \\
$\theta_i^{(t)}$ & Parameter vector of client $i$ at round $t$ \\
$\mathcal{C}_s \subset \mathcal{C}$ & Designated sensitive class set \\
$p_i = \Delta_i(\mathcal{C}_s)$ & Sensitive-class concentration of client $i$ \\
$\hat{p}_i$ & Adversary's estimate of $p_i$ \\
$\bar{p} = \frac{1}{n}\sum_j p_j$ & Constant-mean baseline \\
$\mathcal{A}$ & Adversary \\
$\mathcal{K}$ & Adversary knowledge \\
$\mathcal{O}$ & Adversary observations \\
$\mathcal{I}$ & Adversary inference goal \\
$\mathcal{R}$ & Adversary restrictions \\
$\varepsilon$ & DP privacy budget \\
$\Theta = \{\theta_i^{(t)} : i \in [n], t \in [T_{\max}]\}$ & Full tensor of observed parameter updates \\
$L_{\text{cal}} = \frac{1}{n}\sum_i (\hat{p}_i - p_i)^2$ & Calibration loss \\
$\xi_i^{(t)} \sim \mathcal{N}(0, \sigma_i^2 C^2 I)$ & Per-round Gaussian noise for client $i$ at round $t$ \\
$C$ & Gradient clipping threshold \\
$\mathcal{F}_{\mathcal{G}, \omega}$ & Family of priors over client distributions \\
$\ell_i^\circ$ & Structural leverage of client $i$ \\
$U$ & Total noise variance budget \\
$K^\star$ & Optimal worst-case MI bound \\
$K_{\mathrm{uniform}} = an/U + \max_i \ell_i^\circ$ & Worst-case MI bound under uniform allocation \\
$a = T_{\max} C^2 / (2|B|^2)$ & Shorthand constant for allocation \\
$|B|$ & Batch size \\
$\mathcal{A}_0$ & Constant-mean baseline adversary \\
$\mathcal{A}_1$ & Parameter-only adversary \\
$\mathcal{A}_2^{\text{topo}}$ & Structural adversary \\
$\mathcal{A}_2^{\text{org}}$ & Organisational adversary \\
$\mathcal{A}_2^{\text{full}}$ & Full-knowledge adversary \\
$f_\phi$ & Adversary's regressor with learnable parameters $\phi$ \\
$x_i \in \mathbb{R}^k$ & Structural and organisational feature vector for client $i$ \\
$k$ & Dimension of structural and organisational feature vector \\
$\text{top-}k(\cdot)$ & Set of $k$ clients with highest sensitive-class concentration \\
$\tau$ & Classification threshold for AUROC \\
$\mathrm{Var}(p)$ & Variance of sensitive-class concentrations across clients \\
$\mathcal{S} = \{(\Theta_i, x_i, p_i)\}_{i=1}^{N}$ & Shadow corpus of labelled tuples \\
$N$ & Number of simulated shadow federations \\
$\mathcal{B}$ & Regressor backend \\
$\Theta^{\text{tgt}}$ & Parameter tensor observed from the target federation \\
$\phi_{\Theta}^{(i)}$ & Parameter feature vector extracted from $\Theta_i$ \\
$\phi_x^{(i)}$ & Structural and organisational feature vector for client $i$ \\
$\mathbb{P}_{\mathrm{world}}$ & World prior generating client datasets \\
$\delta(D_i)$ & Class distribution induced by dataset $D_i$ \\
$\alpha$ & Learning rate \\
$B_i^{(t)}$ & Mini-batch sampled by client $i$ at round $t$ \\
$\ell$ & Loss function \\
$\tilde{g}_i^{(t)}$ & Clipped and noised gradient of client $i$ at round $t$ \\
$\eta$ & Topology-data coupling parameter (Setting~C) \\
$q = |B|/|\mathcal{D}_i|$ & Poisson subsampling rate for client $i$ \\
$D_{-i}$ & Datasets of all clients other than client $i$ \\
$\sigma_i^{*\,2} = a/(K^\star - \ell_i^\circ)$ & Optimal per-client noise variance \\
$\ell_i^{\mathrm{org}} \propto |G_i|$ & Group-size leverage proxy \\
$\ell_i^{\mathrm{deg}} \propto \deg_{\mathcal{G}}(i)$ & Degree leverage proxy \\
$\ell_i^{\mathrm{ds}} \propto |\mathcal{D}_i|/\bar{|\mathcal{D}|}$ & Dataset-size leverage proxy \\
$H(\Phi_{\omega_i})$ & Block entropy under SBM prior \\
$|G_i|$ & Size of organisational group containing client $i$ \\
\bottomrule
\end{tabular}
\end{table}
