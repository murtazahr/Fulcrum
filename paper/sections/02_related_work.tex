...
\emph{complementary} threat — most relevant when DP is weak — and include
it as an empirical baseline in our evaluation.
...
\emph{neighborhood-aggregated} updates rather than per-client ones; this
weakens \TADI's parameter channel without addressing the structural and
organisational channels. We evaluate this regime as an extension
experiment (\S\ref{sec:experiments}).
...
isolating parameter, structural, and organisational information; (ii)
prove a tight MI bound that decomposes into a controllable mechanism
term plus an uncontrollable prior-coupling floor; and (iii) derive a
closed-form noise-allocation policy that strictly improves on uniform
DP-SGD whenever the topology breaks leverage symmetry, with three
principled proxies for instantiating leverage in deployed regimes.
