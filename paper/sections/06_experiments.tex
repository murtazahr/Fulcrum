...
the public FLamby data --- the only shadow construction available to
an adversary that knows only public protocol parameters and does not
have per-site labels (the Stage~\ref{sec:adv-model} threat model).
The target uses FLamby's native hospital-site partitioning. The
prior families therefore differ by construction: synthetic
Dirichlet at shadow time, real cross-silo demographics at test time.
\end{itemize}
...
channel's positive lift is therefore the empirical manifestation of
$\ell_i^\circ$ being achievable when adversary's shadow prior matches
target's deployment prior --- a prerequisite that Setting~C satisfies
by construction and Settings~B and~A do not.
...
asymmetry doesn't correlate with $p_i$ here. This is the
\emph{decomposition} we set out to characterise: the channel that
matters depends on the deployment.
\end{itemize}
...
target distribution). Per the Stage~\ref{sec:adv-model} threat model,
...
CV-tuned regressor). $\Adv_2^{\text{org}}$ and $\Adv_2^{\text{topo}}$
show no variance across target runs because their inputs (omega
one-hot and ring-topology degree features) are identical across all
108 target runs; their $K^\star$ correlations are accordingly
undefined.}
...
on the per-client noise allocation drives the controllable mechanism
term in Theorem~\ref{thm:per-client} low enough that the regressor
cannot beat constant-mean baseline regardless of which shadow prior
it was trained on. This is the most robust empirical finding in the
paper, and it holds across both the synthetic-vehicle and realism-
anchor regimes.

Second, \textbf{the prior-coupling channel is not empirically
realised.} The full-channel $\Adv_2^{\text{full}}$ attack reaches
AUROC $= 0.56$ --- positive discriminative power, just past the
random-prediction threshold --- but its attack lift on the
calibration-loss metric remains negative ($-0.041$). The regressor
extracts \emph{some} signal that ranks target sites' $p_i$ slightly
better than chance, but not enough signal to lower calibration loss
below the trivial constant-mean baseline. This is consistent with
...
architecture --- three orthogonal methodological improvements
(DP-matched shadow, class-aware features, CV-tuned regressor) drove
$\Adv_1$ lift from $-0.097$ to $-0.058$ and $\Adv_2^{\text{full}}$
AUROC from $0.19$ to $0.56$, demonstrating that within-pipeline
optimisation does carry signal across the gap. The residual
negativity is the empirical signature of a \emph{structural} prior
gap between public-proxy shadow training and native cross-silo
targets.
...
The combined result is that \emph{Fulcrum's bound is valid as an
upper bound, tight in the prior-matched regime, and conservative
in the public-proxy regime}. The defense's empirical efficacy
(Pareto dominance, utility consistency, controllable-term bounding)
is independent of which regime applies.
