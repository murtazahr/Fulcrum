...
\item $\Adv_2^{\text{full}}$ does not strictly dominate
  ...
  claim the four-channel adversary is universally the worst case. A
  deeper alternative reading is that finite-shadow noise interference
  is itself a constraint on adversary capability: even granted the
  full $\KNW=(\TOPO,\omega,\{\sigma_j\})$ knowledge of the threat
  model, a regressor trained on a realistically-sized public corpus
  may underperform a single-channel specialist, further widening the
  gap between $\ell_i^\circ$ and realised adversary capability. This
  reinforces the deployment-favourable conservatism of the bound.
...
\paragraph{Observation (Prior-realisability gap).}
We summarise the cross-setting evidence as a single empirical
observation about when Theorem~\ref{thm:per-client}'s bound is
tight versus loose:

\begin{quote}
\emph{The prior-coupling supremum $\ell_i^\circ$ is empirically
attained when the divergence between the adversary's shadow prior
$\mathbb{P}_{\mathrm{shadow}}$ and the deployment's target prior
$\mathbb{P}_{\mathrm{target}}$ within the family
$\FAM_{\TOPO,\omega}$ is small (Setting~C, where shadow and target
share an $\eta$-coupling adapter and differ only in seed). When that
divergence is large (Settings~B and~A, where shadow uses synthetic
Dirichlet re-partitioning of public data and target uses native
cross-silo sites), the empirical attack lift falls strictly below
$\ell_i^\circ$ and the bound becomes correspondingly conservative.}
\end{quote}

We do not formalise the divergence threshold quantitatively; doing so
would require an analytical relationship between the adversary's
expected attack lift and a divergence measure (KL or TV) on the
prior family $\FAM_{\TOPO,\omega}$, which we leave as future work.
Empirically, the qualitative split is clear: prior matched
$\Rightarrow$ supremum attained; prior mismatched
$\Rightarrow$ supremum unrealised. This observation is independent of
the controllable-mechanism bound (Finding~1), which holds in both
regimes.
