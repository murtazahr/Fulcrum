...
\caption{Setting~C $\eta$-sweep across four topologies. The $x$-axis
is the topology--data coupling $\eta \in [0,1]$; the $y$-axis is the
worst-case bound $K^\star$ (nats); curves show \Fulcrum (blue) and
uniform allocation (red). The IID null at $\eta=0$ gives identical
curves on every topology; the gap grows monotonically with $\eta$ where
leverage is non-uniform, saturates near $an/U$ on the star, and is zero
...
AUROC $= 0.56$, representing positive discriminative power just past
the random-prediction threshold, but its attack lift on the
...
collapses to neighbourhood-aggregated updates. We re-evaluate \TADI in
