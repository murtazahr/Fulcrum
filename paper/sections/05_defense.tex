...
We label the allocation \emph{Fulcrum} because it is the privacy
balance point: per-client privacy bounds are equilibrated across the
asymmetric leverage profile under a fixed utility budget, the way an
asymmetric lever finds its fulcrum at the balance point of unequal loads.
...
The decomposition is the key structural feature. The first term is
...
prior and is not reduced by DP-SGD noise on client $i$. We design the
allocation to manage the controllable term given that the uncontrollable
floor is fixed.
...
The corollary characterises both when our allocation helps and when it
collapses to standard DP-SGD. Symmetric topologies — ring, complete,
balanced hierarchical — produce uniform leverage and our allocation
reduces to uniform DP-SGD; this is the appropriate behaviour, not a
flaw, since under those topologies there is no asymmetry to exploit.
Asymmetric topologies — line, hierarchical with uneven groups, star — and
deployments with heterogeneous client dataset sizes both produce
non-uniform leverage and admit a strict gap.
...
prior-dependent constant. Use the proxy
$\ell_i^{\mathrm{org}} \propto |G_i|$ in hierarchical FL with uneven
groups (\eg Briggs et al.\ clustering~\cite{briggs2020hierarchical}).
\end{corollary}
...
substantially — the typical FLamby regime~\cite{flamby2022}. Like
...
\begin{table}[h]
\centering
\caption{When the topology-aware allocation strictly improves over
uniform DP-SGD. Empirical validation in \S\ref{sec:experiments}.}
\label{tab:regimes}
\begin{tabular}{@{}lll@{}}
\toprule
Topology / structure & Leverage uniform? & Strict gap over uniform? \\
\midrule
Ring (uniform degree) & Yes & No (degenerate; \cf~Cor.~\ref{cor:strict}) \\
Complete (uniform degree) & Yes & No (degenerate) \\
Line (asymmetric endpoints) & No & Small (typically $<0.05$ nats) \\
Hierarchical with balanced groups & Yes & No (degenerate) \\
Hierarchical with uneven groups & No & Moderate (depends on size variance) \\
Star (1 hub + $n-1$ leaves) & No & Large; saturates at $\sim an/U$ \\
Cross-silo with heterogeneous $|\Dset_i|$ & No & Strong (\cf~Cor.~\ref{cor:influence}) \\
\bottomrule
\end{tabular}
\end{table}

\PLACE{Replace the qualitative entries in Table~\ref{tab:regimes} with
measured gap values once Stage~6 sweeps complete.}
