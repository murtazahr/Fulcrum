...
lemmas plus an assembly step. The proof uses standard machinery — chain
rule of mutual information, Mironov's RDP for the Gaussian
...
non-smooth so we reformulate via slack variable $K$:
\begin{equation}
...
\(g(K) \to \infty\) as $K \to \max_i \ell_i^\circ^+$ and
...
$U > n a/(K - \ell_{\max})$ for the relevant $K$. For very small $U$
the optimum saturates with some $\sigma_i^2 \to 0$ at low-leverage
clients and most of the budget concentrated at high-leverage ones; in
practice this regime is below the deployment-relevant range and we do
